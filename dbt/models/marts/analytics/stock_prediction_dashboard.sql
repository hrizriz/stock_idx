...
WITH stock_data AS (
    SELECT *
    FROM {{ ref('fct_daily_stock_metrics') }}
),
...
technical_indicators AS (
    SELECT 
        ...
        m.macd_signal,
        m.macd_histogram
    FROM {{ ref('technical_indicators_rsi') }} r
    JOIN {{ ref('technical_indicators_macd') }} m 
        ON r.symbol = m.symbol AND r.date = m.date
),

news_sentiment AS (
    SELECT *
    FROM {{ ref('news_sentiment_analysis') }}
),

lstm_data AS (
    SELECT *
    FROM {{ ref('fct_stock_predictions') }}
),
